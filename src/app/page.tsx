...
export default function LogoPlacerPage() {
  ...
  const [prompt, setPrompt] = useState<string>(
    "A photorealistic image. The placed logo is perfectly integrated into the background. Match the lighting, texture, and perspective of the scene. Add a subtle, realistic shadow. The main product remains untouched. The logo's design is perfectly preserved."
  );
  ...
  const handleFileChange = (
    originalSetter: (img: HTMLImageElement | null) => void,
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          originalSetter(img);
          // Set both initially to make the preview instant
          processedSetter(img);
        };
  ...
  useEffect(() => {
    const processLogo = async () => {
      ...
      if (removeLogoBg) {
        ...
        try {
          const removeBackgroundFunc = (await import('@imgly/background-removal')).default;
          const blob = await removeBackgroundFunc(originalLogo.src);
          const url = URL.createObjectURL(blob);
          const img = new Image();
          img.crossOrigin = "anonymous";
          img.onload = () => setProcessedLogo(img);
          img.src = url;
        } catch (e) {
          setError('Failed to remove background.');
          console.error(e);
          setProcessedLogo(originalLogo);
        } finally {
  ...
  useEffect(() => {
    ...
    const logoToDraw = processedLogo;
    if (logoToDraw) {
  ...
  }, [productImage, processedLogo, logoSize, logoPosition]);
  ...
  const findClosestAllowedSize = (width: number, height: number): [number, number] => {
    ...
    ALLOWED_DIMENSIONS.forEach(dim => {
      ...
      if (diff < minDiff) { minDiff = diff; bestMatch = dim; }
    });
    return bestMatch as [number, number];
  };

  const handleGenerate = async () => {
    if (!productImage || !processedLogo) {
    ...
    try {
      ...
      const logoHeight = processedLogo.height * (logoWidth / processedLogo.width);
      const logoX = offsetX + newWidth * (logoPosition.x / 100) - logoWidth / 2;
      const logoY = offsetY + newHeight * (logoPosition.y / 100) - logoHeight / 2;
      ctx.drawImage(processedLogo, logoX, logoY, logoWidth, logoHeight);
    ...
    <div className="bg-slate-900 min-h-screen text-white font-sans">
      <div className="container mx-auto p-4 sm:p-8">
        <header className="text-center mb-8">
          ...
          <p className="text-slate-400 mt-2">Powered by Stability AI + IMG.LY</p>
        </header>
          ...
          <section className="bg-slate-800/30 p-6 rounded-xl shadow-lg flex flex-col gap-6">
            ...
            {originalLogo && (
              <div className="bg-slate-800/50 p-4 rounded-lg flex items-center justify-between">
                 <label htmlFor="remove-bg" className="font-semibold text-white flex items-center">
                   AI Background Removal
                   {isBgRemoving && <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-sky-400 ml-3"></div>}
                 </label>
